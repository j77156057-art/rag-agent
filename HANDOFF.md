...
- `platform: github` 现在优先走 GitHub Search API，返回仓库、描述、Stars、更新时间和来源 URL；API 限流/网络失败时保留明确诊断，可改用通用站点搜索。
- `platform: b站` 现在优先走 B 站视频搜索 API，返回 BV 链接和摘要；新增 `web_subtitles` 工具，可读取公开 BV/av 视频字幕，登录、验证码、无字幕时返回可审计原因。
- 通用结果增加可解释的可信度排序参考（HTTPS、原站、文档域名、官方/文档标题），明确标注“仅供排序，需核对正文”，不把评分当作事实判断。
- `web_research` 增加轻量数字冲突提示；只标记不同来源出现的同单位不同数值，不自动裁决冲突。
...
| 2026-09-18 | **引擎热重载后续增强（当前提交）**：新增 `GET /api/engine/changes`，引擎启动时记录 `.gd/.tscn/.tres/.shader/.gdextension/.godot/.cfg/.json`（含 `project.godot`）的轻量基线，运行期间只读比较修改/新增/删除文件；忽略 `.godot`、构建目录、依赖目录和缓存，避免无关变更刷屏。`SceneRuntimePanel` 每 2.5 秒轮询并提示"检测到外部修改"，用户确认后手动点击热重载，默认不会自动重启。新增 2 个引擎生命周期测试与路由护栏。验证：Python 全量 **941 项通过、1 项跳过**；`npm run typecheck` 与 `npm run build` 通过。真实桌面 Godot 修改文件→重载闭环仍需本机实测。 |
| 2026-09-18 | **`c9c77de` Agent 自验证闭环收口（Phase 3 跨会话经验记忆 + Phase 4 评测门扩展）**：① **Phase 3 跨会话经验记忆**：新增 `experience.py`（独立 `docmind_experience` 向量集合、脱敏 lesson 文本、project_id 隔离、sha1 去重、容量上限 `DOCMIND_EXPERIENCE_MAX` 默认 200、TTL 置信衰减 fail-then-fixed 30d / repeated-fail 14d / success 90d、证据优先）；新增 `recall_experience(query?)` 工具并注册 `TOOLS`；`agent.py` 回合收尾按 `_derive_experience_outcome` 记录（fail-then-fixed / repeated-fail，成功路径受 `DOCMIND_EXPERIENCE_SUCCESS` 默认关）、回合开始按 `DOCMIND_EXPERIENCE_RECALL` 默认关向系统提示注入 top-5 建议性历史经验（标 stale/低置信、不盲目照搬）；`agent_trace.Turn` 增 `failure_count/max_tool_streak/max_consec_failures/last_failure/experience` 字段；所有外部依赖 try/except 优雅降级，经验链路永不阻断主流程。② **Phase 4 评测门扩展**：golden 题库新增 **G12**（写坏 .py→self_verify 失败→修复→复验通过，`must_call` create_file+self_verify）与 **G13**（先 recall_experience 再说明「证据优先、不照搬过期经验」，`must_call` recall_experience、`must_not_include` 直接照搬/无条件执行/必须执行/照搬结论）；`agent_eval.golden_gate_ok()` 冻结发布门（scored>0 且 failed==0，有 baseline 还需无回归）。③ 文档：`docs/agent-self-verify-memory.md` §6 路线图 Phase 3/4 标记 ✅。验证：新增 `tests/test_experience.py` **33 例**；全量 **1116/1116（0 失败）**、无回归（基线 1083→1116）。注：自动召回/成功记录默认关闭以保基线，仅 golden 评测显式开启；`verify_engine_embed.py`(60) 真机 GUI 验收本会话未跑。 |
...
已完成：web_search（DuckDuckGo）、web_fetch（公开 HTML 正文读取）、web_research（搜索+最多 3 个来源抓取）、GitHub/B 站专用搜索、公开 B 站字幕尝试、摘要缓存、来源排序参考、数字冲突提示、标题/最终 URL/正文清理、非 HTML 与网络失败降级、回答中的来源 URL 可点击。详见 2026-09-18「联网研究能力补齐」条目。

仍未完善：前端独立来源卡片与正文展开、登录/验证码/付费墙处理、跨来源语义级事实核验。当前能力适合带证据的教程、GitHub 资料和引擎文档研究，评分和冲突提示不能替代人工核对，也不能宣称达到 Perplexity/Claude Research 级别。
