...
const running = ref(false), busy = ref(false), logs = ref<string[]>([]), errors = ref<{path:string;line:number;message:string}[]>([]), comfyUrl = ref('http://127.0.0.1:8188'), comfyState = ref('未检测'), workflow = ref(''), comfyResult = ref(''), promptId = ref(''), outputs = ref<{filename?:string;subfolder?:string;type?:string;preview_url?:string;mime?:string}[]>([]), comfyTemplates = ref<{id:string;name:string;model:string;kind:string;workflow?:string}[]>([]), comfyHistory = ref<string[]>(JSON.parse(localStorage.getItem('docmind.comfy.history') || '[]'))
...
async function pollComfy() { if (!promptId.value) return; const r = await comfyApi.history(promptId.value, comfyUrl.value); outputs.value = r.outputs || []; const p = r.progress; comfyResult.value = r.done ? `生成完成（${outputs.value.length} 个结果）` : `生成中 ${p ? `${p.percent}%（${p.executed_nodes}/${p.total_nodes} 节点）` : ''}` }
...
<template>
  <div class="te-panel">
    ...
    <div v-if="open" class="te-pop">
      ...
      <div class="te-comfy"><b>ComfyUI 资源</b><input v-model="comfyUrl" @change="checkComfy" /><div class="te-templates"><button v-for="t in comfyTemplates" :key="t.id" @click="loadComfyTemplate(t.id)">{{ t.name }}</button></div><div v-if="comfyHistory.length" class="te-history"><button v-for="id in comfyHistory" :key="id" @click="selectComfyHistory(id)">{{ id.slice(0,8) }}</button></div><textarea v-model="workflow" placeholder="粘贴 workflow JSON" /><button @click="queueComfy">提交生成</button><button v-if="promptId" @click="pollComfy">查询结果</button><button v-if="promptId" class="te-cancel" @click="cancelComfy">取消生成</button><span>{{ comfyState }} {{ comfyResult }}</span></div>
...
.te-outputs{margin-top:6px;display:flex;flex-direction:column;gap:6px;max-height:180px;overflow:auto}.te-output{display:flex;align-items:center;gap:6px;padding:4px;background:var(--bg);border:1px solid var(--border);border-radius:4px;font-size:10px}.te-output img{width:54px;height:54px;object-fit:cover;border-radius:3px}.te-output audio,.te-output video{max-width:160px;max-height:60px}.te-output span{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:var(--font-mono)}.te-output button{background:#17304b;border:1px solid #315c86;color:#b9d8f5;border-radius:4px;padding:3px 7px;cursor:pointer}
.te-unreal{margin-top:8px;padding-top:8px;border-top:1px solid var(--border);font-size:11px;color:var(--text-muted);display:flex;gap:6px;align-items:center}.te-unreal small{margin-left:2px}.te-unreal button{margin-left:auto;background:#17304b;border:1px solid #315c86;color:#b9d8f5;border-radius:4px;padding:4px 7px;cursor:pointer}.te-unreal-list{max-height:110px;overflow:auto;margin-top:4px;padding:5px;background:var(--bg);font:10px var(--font-mono);color:var(--text-muted)}
</style>
